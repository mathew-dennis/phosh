...
struct _PhoshHome
{
  ...
  GtkGesture     *powerbar_press_gesture;
...
update_drag_handle (PhoshHome *self, gboolean commit)
{
  ...
  if (gtk_gesture_get_sequence_state ((self->osk_toggle_long_press), GdkEventSequence *sequence) == GTK_EVENT_SEQUENCE_DENIED) {
...
static void
powerbar_action_started (PhoshHome *self)
{
...
static void
powerbar_action_ended (PhoshHome *self)
{
...
static void
powerbar_action_failed (PhoshHome *self)
{
...
on_powerbar_pressed (PhoshHome *self, PhoshOskManager *osk, PhoshShell *shell)
{
  ...
  if (osk_is_available) {
    osk_new_state = !osk_current_state;
    /* handle ui changes */
    powerbar_action_ended (self);
  } else {
    powerbar_action_failed (self);
    return;
  } 

  if (osk_is_available && osk_new_state)
    g_signal_emit(self, signals[OSK_ACTIVATED], 0);

  if (osk_current_state != osk_new_state) {
...
phosh_home_constructed (GObject *object)
{
  ...
  g_object_set_data (G_OBJECT (self->powerbar_press_gesture), "phosh-home", self);
}
